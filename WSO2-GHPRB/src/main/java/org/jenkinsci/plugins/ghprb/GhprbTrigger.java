...
/**
 * @author Honza Brázdil <jbrazdil@redhat.com> updated by Himan Gamage
 *         <Himan@wso2.com>
 */
public class GhprbTrigger extends Trigger<AbstractProject<?, ?>> {
	...
	public GhprbTrigger(String adminlist, String whitelist, String orgslist,
			...
			Boolean autoCloseFailedPullRequests, Boolean autoMergePullRequests,
	...
	public void start(AbstractProject<?, ?> project, boolean newInstance) {
		if (project.getProperty(GithubProjectProperty.class) == null) {
			logger.log(
					Level.INFO,
					"GitHub project not set up, cannot start trigger for job {0}",
					project.getName());
			return;
		}
		try {
	...
	public Ghprb createGhprb(AbstractProject<?, ?> project) {
		return Ghprb.getBuilder().setProject(project).setTrigger(this)
	...
	public QueueTaskFuture<?> startJob(GhprbCause cause, GhprbRepository repo) {
		...
		values.add(new StringParameterValue("ghprbPullAuthorEmail", cause
				.getAuthorEmail() != null ? cause.getAuthorEmail() : ""));
		values.add(new StringParameterValue("ghprbPullDescription", String
				.valueOf(cause.getShortDescription())));
		values.add(new StringParameterValue("ghprbPullTitle", String
				.valueOf(cause.getTitle())));

		String prUrl = cause.getUrl() != null ? cause.getUrl().toString()
				: String.valueOf(repo.getRepoUrl() + "/pull/"
						+ cause.getPullID());
...
//					this.getGhprb().getBuilds());
			...
			this.getGhprb().getBuilds()
					.setParameters(new ParametersAction(values));
	...
	private AbstractBuild findCauseProject(AbstractBuild<?, ?> currentBuild) {
		...
		Outer: for (Cause c : currentBuild.getCauses()) {
			...
			if (!(c instanceof UpstreamCause)) {
				...
				testBuild = findCauseProject(upBuild);
			}
		}
		return testBuild;
	}
	...
	public static final class DescriptorImpl extends TriggerDescriptor {
		...
		public FormValidation doCheckServerAPIUrl(@QueryParameter String value) {
			...
			if (value.endsWith("/api/v3"))
				return FormValidation.ok();
			return FormValidation
					.warning("GitHub api url is \"https://api.github.com\". GitHub enterprise api url ends with \"/api/v3\"");
		}
		...
		Map<Integer, GhprbPullRequest> getPullRequests(String projectName) {
		...
		public FormValidation doCreateApiToken(
			...
			try {
				GitHub gh = GitHub.connectToEnterprise(this.serverAPIUrl,
						username, password);
