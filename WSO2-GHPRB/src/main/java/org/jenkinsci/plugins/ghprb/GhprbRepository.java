...
public class GhprbRepository {
	...
	public void createCommitStatus(AbstractBuild<?,?> build, GHCommitState state, String message, int id,GhprbCause cause){
		String sha1 = cause.getCommit();
		createCommitStatus(sha1, state, Jenkins.getInstance().getRootUrl() + build.getUrl(), message, id);
	}
	public void createCommitStatus2(GhprbCause cause,AbstractBuild<?,?> build, GHCommitState state, String message, int id){
		String sha1 = cause.getCommit();
		createCommitStatus(sha1, state, Jenkins.getInstance().getRootUrl() + build.getUrl(), message, id);
	}
